...
for(ind in seq_len(nrow(region_season_combos))) {
  ...
  output_path <- "/home/er71a/adaptively-weighted-ensemble/inst/estimation/kcde/cluster-output"
  lsfoutfilename <- "adaptively-weighted-ensemble-kcde-loso-predict.out"
  
  case_descriptor <- paste0(region, "-", gsub("/", "-", season))
  filename <- paste0(output_path, "/submit-kcde-loso-prediction-step-", case_descriptor, ".sh")
  ...
  cat(paste0("R CMD BATCH --vanilla \'--args ",
    region, " ",
    season,
    "\'  /home/er71a/adaptively-weighted-ensemble/inst/estimation/kcde/kcde-prediction-loso.R ",
    output_path, "/output-kcde-loso-prediction-step-", case_descriptor, ".Rout"),
    file = filename, append = TRUE)
