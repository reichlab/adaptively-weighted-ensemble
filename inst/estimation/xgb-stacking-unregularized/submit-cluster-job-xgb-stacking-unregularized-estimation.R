...
for(region in all_regions) {
  for(prediction_target in all_prediction_targets) {
    for(explanatory_variables in all_explanatory_variables_combos) {
      output_path <- "/home/er71a/adaptively-weighted-ensemble/inst/estimation/xgb-stacking-unregularized/cluster-output"
      lsfoutfilename <- "adaptively-weighted-ensemble-xgb-stack-unregularized.out"
      
      case_descriptor <- paste0(
        region,
        "-prediction_target_", prediction_target,
        "-explanatory_vars_", explanatory_variables
      )
      filename <- paste0(output_path, "/submit-xgb-stacking-unregularized-estimation-step-", case_descriptor, ".sh")
      ...
      cat(paste0("R CMD BATCH --vanilla \'--args ",
        ...
        cores_req,
        "\'  /home/er71a/adaptively-weighted-ensemble/inst/estimation/xgb-stacking-unregularized/xgb-stacking-unregularized-estimation.R ",
        output_path, "/output-xgb-stacking-unregularized-estimation-", case_descriptor, ".Rout"),
        file = filename, append = TRUE)
