...
for(ind in seq_len(nrow(region_season_combos))) {
  ...
  output_path <- "/home/er71a/adaptively-weighted-ensemble/inst/evaluation/sarima/cluster-output"
  lsfoutfilename <- "adaptively-weighted-ensemble-sarima-test-predict.out"
  
  case_descriptor <- paste0(region, "-", gsub("/", "-", season))
  filename <- paste0(output_path, "/submit-sarima-test-prediction-step-", case_descriptor, ".sh")
  ...
  cat(paste0("R CMD BATCH --vanilla \'--args ",
    region, " ",
    season,
    "\'  /home/er71a/adaptively-weighted-ensemble/inst/evaluation/sarima/sarima-prediction-test.R ",
