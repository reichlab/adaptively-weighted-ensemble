...
To create an empirical predictive distribution of size $N$ from a KDE fit based on a data vector of length $K$ $\hat \by_{1:K}$, we first drew $N$ samples (with replacement) from $\hat \by_{1:K}$, yielding a new vector $\tilda \by_{1:N}$. 
We then drew $N$ psuedo-random deviates from Gaussian distributions centered at $\tilda \by_{1:N}$ with the bandwidth estimated by the KDE algorithm. 
...
Our implementation for this paper conditioned on recent past incidence and week of season, so only information from the simple time series, although the method would allow for additional covariates to be included.
...
We fit seasonal ARIMA models \citep{Box2015} to wILI transformed to be on the natural log scale. We used the stepwise procedure from the `auto.arima` function in the `forecast` package to select the order of first- and seasonal-differencing as well as the specification of the order of auto-regressive and moving average terms for the SARIMA model.\citep{Hyndman2008} Additionally, data were Box-Cox transformed prior to modeling.
