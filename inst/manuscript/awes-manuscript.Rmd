...
Describe what each model produces...
...
For example, $Y$ could represent the peak incidence for a given season or the incidence in the season corresponding to a particular week, and $\bX$ may include features such as time of year at which the prediction is being made, or other such features of the observed data or the component models.
...
\pi_{m}(\bX) &= \frac{\exp\{\rho_m(\bX)\}}{\sum_{m' = 1}^M \exp\{\rho_{m'}(\bX)\}}  \label{eqn:PiMultilogitRho} 
\end{align}
where $\rho_m(\bX)$ is an estimated function that can depend on observed data. In this set-up, the weight for each model, $\pi_m$ is taken as the softmax of the vector of $\rho_m$ values. This implies that when the value of $\rho_m(\bX)$ is large relative to the other models, then model $m$ would have more weight than the other models. 
...
3. Feature-weighted, unregularized, i.e. $\rho_m(\bX)$ is estimated using gradient boossting with features including week of the season and 
