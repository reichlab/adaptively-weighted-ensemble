...
Using component models that generate predictive densities for outcomes of interest, we have developed a feature-weighted ensemble method that estimates model weights as functions of observed data and aspects of the predictive distributions obtained from those component models. Our approach fuses aspects of different ensemble methods: it uses model stacking \citep{Wolpert1992}, combines predictive densities as in Bayesian Model Averaging \citep{Madigan1998}, and estimates model weights based on features of the system \citep{Sill2009} using gradient tree boosting [[TODO: cite Friedman]].
...
Development of the methods presented in this manuscript was motivated by the observation that certain prediction models for infectious disease consistently performed better than other models at certain times of year.  We observed that early in the U.S. influenza season, simple models of historical incidence often outperformed more standard time-series prediction models such as a seasonal auto-regressive integrated moving average (SARIMA) model. However, in the middle of the season, the time-series models showed improved accuracy. We set out to determine whether ensemble methods could use this information about past model performance to improve predictions.

[[TODO: Insert 1-paragraph ensemble methods literature review here.]]

While there are many different methods for combining models, all ensemble models discussed in this paper use an approach called stacking.  In this approach, each of the component models is trained separately in a first stage, and cross-validated measures of performance of those component models are obtained.  Then in a second stage, a stacking model is trained using those cross-validation results as an input to learn how to combine predictions from the component models.

Using seasonal influenza outbreaks in the US health regions as a case-study, we developed and applied our feature-weighted ensemble model to predicting several attributes of the influenza season at each week during the season.  The remainder of this article is organized as follows.  In the next section, we describe our methods, including a description of our application and the model specifications.  Next, we present the results of our application.  We conclude with a discussion.

# Methods

We begin this section with a discussion of the motivating prediction task for our method development, including a description of the influenza data we use in our application and the prediction targets.
Next, we discuss the three component models that our ensemble builds on.
Finally, we turn to the ensemble methods, describing our ensemble model specification as well as two reference models that we use to indicate baseline ensemble performance in our application.
...
We begin our discussion of ensemble methods with the novel feature-weighted stacking framework that we propose.
We discuss the stacking model specification, estimation, and several variations on the model that we will use in our application.
Next, we describe two simpler stacking approaches that we use as reference ensemble methods for our proposed stacking methodology.

All of these methods work by averaging predictions from the component models to obtain the ensemble prediction.
With this approach, the ensemble predictions are guaranteed to be predictive distributions that integrate to one.
Additionally, all of the ensemble methods we compare are stacked model ensembles because they use leave-one-season-out predictions from the separately-estimated component models as inputs to estimate the model weights (cite friedman stacking paper?).
...
Therefore for models $l, m \in \{1, ..., M\}$, $\rho_l(\bX) > \rho_m(\bX)$ indicates that model $l$ has more weight than model $m$ for predictions at the given value of $\bX$.

## Baseline ensemble methods

We propose a framework for estimating _feature-dependent weights_ for a stacked ensemble model. 
...
```{r log_scores_vs_analysis_time, echo=FALSE, cache=TRUE, fig.height=8, fig.cap="Mean, minimum, and maximum log scores achieved by each component model in each week of the season, summarizing across all seasons in both the train and test phases when all three component models produced predictions."}
...
all_models <- c("kde", "kcde", "sarima", "equal_weights", "em_stacking")
