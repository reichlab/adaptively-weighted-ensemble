...
In this work, we develop and compare a range of ensemble methods for combining predictive distributions in order to explore the benefits ensemble methods of varying complexity may offer for improving the quality of predictions.
...
A large number of ensemble methods have been developed for a diverse array of tasks including regression, classification, and density estimation.  These methods are broadly similar in that they combine results from multiple "base" or "component" models.  However, many details differ between different ensemble methods.  We suggest \cite{polikar2006ensemble} for a review of ensemble methods; many of these methods are also discussed in detail in \cite{Hastie2011}.
...
Several variations on this strategy have been explored in the literature previously \citep{smyth1999stackingDensityEstimators,  rigollet2007linearconvexaggregationdensity, ganti2011cake}.  However, other ensemble methods for density estimation have also been developed.  For example, \citep{rosset2002boosting} develop a boosting method in which the component density estimates are estimated sequentially, with results from earlier models affecting estimation of later models.

In structured prediction settings such as time series forecasting, ensemble methods may benefit from taking advantage of the data structure.  For example, it may be the case that different models offer a better representation of the data at different points in time.  A common idea in these settings is to use model weights that change over time.  For instance, model weights may vary as a function of how well each model did in recent predictions \citep{herbster1998tracking} or by using a more formal graphical structure such as a hidden Markov model to track which component model is most likely to have generated new observations as they arise over time \citep{yamanishi2007dynamicmodelselection, cortes2014ensembleStructuredPrediction}.  It is also possible to combine the component models with weights that depend on observed covariates or features \citep{Sill2009}.  For example, in an ensemble for a user recommendation system, \cite{jahrer2010Netflix} allowed model weights to depend on a variety of features including the time that a user submitted a rating.
...
This approach fuses aspects of different ensemble methods: it uses model stacking \citep{Wolpert1992} and estimates model weights based on features of the system \citep{Sill2009} using gradient tree boosting \citep{friedman2001greedy}.
...
For example, consider predictions of peak timing, which could be used to plan the deployment of medical resources \citep{cdc-decisions-2016}.  Among the component models we considered, \textbf{SARIMA} had the highest mean log score across the test phase, and was the best model in four out of five test-phase seasons.  However, it had the worst performance of all models we considered in the 2015/2016 season.  The \textbf{EW} and \textbf{FW-reg-w} ensembles achieved only slightly lower average log scores than \textbf{SARIMA} overall, were among the best-performing methods in all five test seasons, and assigned an average of about 60 to 70 percent more probability to the eventually realized peak week than \textbf{SARIMA} in predictions made during the season that \textbf{SARIMA} struggled.
