...
We obtained publicly available data on seasonal influenza activity in the United States from the U.S. Centers for Disease Control and Prevention (CDC). 
For each of the 10 Health and Human Services regions in the country, the CDC calculates and publishes each week a measure called the weighted influenza-like illness (wILI) index. 
The wILI for a particular region is calculated by taking the average proportion of doctor visits with influenza-like illness for each state in the region, weighted by state population. 
...
This includes new wILI data from as recent as two weeks prior, as well as updates to previously reported numbers as new data becomes available. 
For this analysis, we use only the final reported wILI


Specifically, we aimed to predict three values defined by the US Centers for Disease Control and Prevention (US CDC) that define important features of an epidemic season: 
