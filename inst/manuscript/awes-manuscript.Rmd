...
\begin{table}[!ht]
\centering
\caption{\label{tbl:ModelPerformanceSummaryTableCombined}Summary of average log scores and mean absolute deviation (MAD) of log scores across all spatial units, test phase seasons, and prediction targets for each method.}
\begin{tabular}{r c c}
\toprule
 Model & Mean & Minimum \\
\hline
...
preds <- preds %>%
  left_join(preds_median_ls_by_model_target, by = c("model", "prediction_target")) %>%
...
#  gather("statistic", "value", mean_diff_from_median, min_diff_from_median)

for(model_val in c("KDE", "KCDE", "SARIMA", "EW", "CW", "FW-wu", "FW-reg-w", "FW-reg-wu", "FW-reg-wui")) {
  cat(model_val)
  cat(" & ")
  cat(format(round(as.numeric(realized_res[model_val, 2]), 2), nsmall = 2))
  cat(" & ")
  cat(format(round(as.numeric(realized_res[model_val, 3]), 2), nsmall = 2))
#  cat(" & ")
#  cat(format(round(as.numeric(realized_res[paste0(model_val, "_var"), 1]), 2), nsmall = 2))
  cat(" \\\\\n")
}
```
\bottomrule
...
perm_test_res <- foreach(i = seq_len(nrow(m_combos))) %dopar% {
  ...
  perm_res <- rbind.fill(lapply(seq_len(10), function(j) {
    perm_model_scores <- readRDS(
#          file = paste0("../../../inst/supplement/score-permutations/perm_model_scores_season_diff_from_median_",
            file = paste0("~/Documents/research/epi/ensembles/adaptively-weighted-ensemble/inst/supplement/score-permutations/perm_model_scores_season_diff_from_median_",
            model_pair[1],
            "_", model_pair[2],
            "_", j,
            ".rds"))
...
```{r perm_test_means_plot, echo=FALSE, fig.height=8.5, fig.cap="\\label{fig:pvals}Differences in mean log scores (panel A) and mean absolute deviations (MAD) of log scores (panel B) across all three prediction targets and all 55 combinations of region and season in the test phase, for predictions made before the target event (onset or peak) occurred.  Differences are for the model on the vertical axis minus the model on the horizontal axis.  Higher mean log scores are better; a positive difference in means (blue) indicates that the model on the vertical axis outperformed the model on the horizontal axis.  For a fixed value of mean log scores, lower MAD is better; a negative difference in MAD (blue) indicates that the model on the vertical axis outperformed the model on the horizontal axis.  Within each cell, the difference in mean or MAD log scores and an approximate p-value derived from a permutation test (see Supplement) are displayed.  For reference, a Bonferroni correction at a familywise significance level of 0.05 within each panel leads to a significance cutoff of approximately 0.0014."}
stat_ind <- 1
...
p_res <- ggplot(
  ...
  xlab("Model") +
  ylab("") +
  ggtitle("A: Mean Difference in Log Scores from Median Method") +
  theme_bw() +
...
#    high = "#b2182b",
  ...
  xlab("Model") +
  ylab("Model") +
  ggtitle("B: Pairwise differences in\nMean Difference in Log Scores from Median Method") +
  theme_bw() +
...
```{r mad_perm_test_plot, echo = FALSE, fig.height=8.5, fig.cap="\\label{fig:pvals}Differences in mean log scores (panel A) and mean absolute deviations (MAD) of log scores (panel B) across all three prediction targets and all 55 combinations of region and season in the test phase, for predictions made before the target event (onset or peak) occurred.  Differences are for the model on the vertical axis minus the model on the horizontal axis.  Higher mean log scores are better; a positive difference in means (blue) indicates that the model on the vertical axis outperformed the model on the horizontal axis.  For a fixed value of mean log scores, lower MAD is better; a negative difference in MAD (blue) indicates that the model on the vertical axis outperformed the model on the horizontal axis.  Within each cell, the difference in mean or MAD log scores and an approximate p-value derived from a permutation test (see Supplement) are displayed.  For reference, a Bonferroni correction at a familywise significance level of 0.05 within each panel leads to a significance cutoff of approximately 0.0014."}
stat_ind <- 2
stat_val <- unique(perm_test_summaries$statistic)[stat_ind]
