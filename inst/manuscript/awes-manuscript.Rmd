...
```{r log_scores_and_weights_vs_analysis_time, echo=FALSE, cache=TRUE, fig.height=3.5, fig.cap="\\label{fig:example-weights}Example of component model weights from the \\textbf{CW} and \\textbf{FW-reg-w} models.  Panel A shows mean, minimum, and maximum log scores achieved by each component model for predictions of season onset timing at the national level in each week of the season, summarizing across all seasons in the training phase when all three component models produced predictions.  Panel B shows log scores from the \\textbf{CW} and \\textbf{FW-reg-w} ensemble methods at each week in the season."}
...
p_weights <- ggplot() +
  ...
  theme(legend.position = "none",
    plot.margin = unit(c(6, 6, 6, 10), "pt"))

component_legend <- get_legend_grob(p_scores)
p_scores <- p_scores +
  theme(legend.position = "none")
...
grid.text("Component Model Weights and Training Phase Log Scores\nNational, Season Onset",
  gp = gpar(fontsize = 14),
  vp = viewport(layout.pos.row = 1, layout.pos.col = 1:2))
print(p_scores, vp = viewport(layout.pos.row = 2, layout.pos.col = 1))
print(p_weights, vp = viewport(layout.pos.row = 3, layout.pos.col = 1))
