...
Although the aggregate performance of these models is quite similar, some differences between the methods begin to emerge when we examine performance in more detail.  It is important that these predictions be of consistent quality in all seasons, whether the seasonal dynamics follow historic seasonal trends or diverge from those common patterns.

We evaluated the consistency of predictive performance using a measure of each model's performance relative to the model with median performance in each combination of the 3 prediction targets, 11 spatial units, and 5 test-phase seasons.
This measure was calculated in two stages.
First, for each model, prediction target, region, and test phase season, we calculated the mean log score for all predictions made before the event being predicted (season onset or season peak) occurred.
We then standardized these values by calculating the difference in mean log scores for a given method and the method with median performance within each combination of prediction target, region, and test phase season.
These quantities describe how each model performed relative to the median model, accounting for variation in prediction difficulty across different region-seasons and prediction targets.


; second, we calculate the average of these differences across all prediction targets, regions, and test phase seasons.  A positive value indicates that on average, the given model outperformed the median model.

relative performance of predictions from each model that were generated before the onset or peak week in each test season (Fig \ref{fig:test_phase_log_scores_heatmap}).
