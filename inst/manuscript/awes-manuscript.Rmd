...
To create an empirical predictive distribution of size $N$ from a KDE fit based on a data vector of length $K$ ($\hat \by_{1:K}$), we first drew $N$ samples (with replacement) from $\hat \by_{1:K}$, yielding a new vector $\tilde \by_{1:N}$. 
...
For the onset week outcome, we added the probability of no onset occuring within a season as [[XXXX]] and then standardized this along with the sampled probabilities so they summed to 1.
...
Our implementation for this paper conditioned on recent past incidence and week of season, only using information from the time series itself, although the method would allow for additional covariates to be included.
...
Predicitive densities for the seasonal quantities can be obtained as appropriate integrals of this joint density (see \citep{Ray2016} for details).

[[Optional: add 2-3 sentences with a few more details on methodology, etc... cross-validation, bandwidth selection, periodic kernel, data on log-scale?, etc...]]
...
(Several of the training seasons had 33 weeks per season because they had a MMWR week 53.)
...
<!-- Is this capturing all of the subtlety of rounding in bins? -->
...
This paper proposes a novel framework for estimating feature-dependent weights for a "stacked" ensemble model, i.e. an average of models. 
In a general context, by "feature-dependent" we mean that the weights assigned to different component models are assumed to have an relationship to observed features or covariates that can be estimated.
(Although we illustrate the method in a the context of time-series predictions, it could also be used in any context where predictive distributions are being combined.)
Features can be observed data from the time-series being predicted (such as recent wILI measurements or the time of year at which predictions are being made), observed data from outside the focal time-series (such as, for example, recent weather observations), or features of the predictions themselves (e.g. summaries of the predictive distribution itself, such as a measure of spread in the distribution, or the time since a predicted peak).
Based on exploration of training phase data only and a priori knowledge of the disease system, we chose three features of the system to illustrate the proposed "feature-weighting" methodology: week of season, model confidence (defined as the minimum number of predictive distribution bins required to cover 90% probability), and wILI measurement at the time of prediction.

To ensure that we could evaluate variations of the feature-weighted ensemble fairly, we also developed several simple alternatives for model averaging so that the proposed methods could be compared with easier-to-implement options. 
All of the ensemble frameworks used can be described using the a single set of notation.
...
Using component model predictions from the training phase, we used four distinct techniques to create "stacked" models, or a probabilistic ensemble prediction. 
