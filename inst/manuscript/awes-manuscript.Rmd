...
```{r log_scores_vs_confidence_v2, echo=FALSE, cache=TRUE, fig.height=8, fig.cap="Log scores vs. confidence v2: log scores achieved by each component model vs. model confidence, all seasons in both the train and test phases when all three component models produced predictions."}
color_palette <- c("#E69F00", "#56B4E9", "#009E73")
...
```{r test_phase_log_scores_summary_v1, echo=FALSE, cache=TRUE, fig.height=8, fig.cap="Test phase log scores summary v1: log scores for all predictions in a season summed.  Log scores of -Infinity are not represented."}
...
preds_sum <- preds %>%
  group_by(model, analysis_time_season, prediction_target) %>%
  summarize(season_log_score = sum(log_score))

ggplot() +
  geom_point(aes(x = analysis_time_season, y = season_log_score, colour = model, shape = model),
    data = preds_sum) +
  geom_line(aes(x = analysis_time_season, y = season_log_score,
      colour = model, linetype = model, group = model),
    data = preds_sum) +
  # geom_point(aes(x = analysis_time_season, y = season_log_score),
  #   colour = "red",
  #   shape = "+",
  #   size = 5,
  #   position = position_jitter(width = 1, height = 0),
  #   data = preds_sum %>%
  #     filter(is.infinite(season_log_score)) %>%
  #     mutate(log_score = -1000)) +
...
```{r test_phase_log_scores_summary_v3, echo=FALSE, cache=TRUE, fig.height=8, fig.cap="Test phase log scores summary v3.  Log scores across all seasons/season weeks represented in box plots.  Log scores of -Infinity are represented with a plus sign at -10."}
preds <- assemble_predictions(
  preds_path = "../../inst/evaluation/test-predictions",
  models = c("kde", "kcde", "sarima", "equal_weights", "em_stacking"),
  prediction_targets = c("onset", "peak_week", "peak_inc"),
