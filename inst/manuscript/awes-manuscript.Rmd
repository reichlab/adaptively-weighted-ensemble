...
Accurate and reliable predictions of infectious disease dynamics would be valuable to public health organizations planning interventions to decrease or prevent disease transmission.  
A great variety of models have been developed for this task, using different model structures and covariates. Experience has shown that the performance of these models varies; some tend to do better or worse in different seasons or at different points within a season. 
Ensemble methods combine predictions from multiple models to obtain a single prediction that leverages the strengths of each model.
We considered a range of ensemble methods that form a predictive density for a target of interest as a weighted sum of the predictive densities from component models. 
...
The international significance of emerging epidemic threats in recent decades has increased public awareness of the importance of understanding and being able to predict infectious disease dynamics.
...
A large number of ensemble methods have been developed for a diverse array of tasks including regression, classification, and density estimation.  These methods are broadly similar in that they combine results from multiple "base" or "component" models.  However, details differ between ensemble methods.  We suggest \cite{polikar2006ensemble} for a review of ensemble methods; many of these are also discussed in detail in \cite{Hastie2011}.
...
Several variations on this strategy have been explored in the literature previously \citep{smyth1999stackingDensityEstimators,  rigollet2007linearconvexaggregationdensity, ganti2011cake}.  However, other ensemble methods for density estimation have also been developed.  For example, \citep{rosset2002boosting} develop a boosting method in which the component models are estimated sequentially, with results from earlier models affecting estimation of later models.
...
These bins were used in the 2016-2017 influenza prediction contest run by the CDC [[TODO: cite something]].
...
In those cases, we calculated the log score as the log of the sum of the probabilities assigned to those weeks; this is consistent with scoring procedures used in the 2016-2017 flu prediction contest run by the CDC [[TODO: cite something]].
