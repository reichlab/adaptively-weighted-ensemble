...
Figure \ref{fig:ComponentModelLogScoresVsWeek} displays variation in leave-one-season-out log scores from the three component models over the course of the training phase seasons, along with the corresponding model weight estimates from the \textbf{CW} and \textbf{FW-reg-w} models.  In general, average performance of the SARIMA and KCDE models track each other closely over the course of the season, with performance improving as more data become available.  In the training seasons, average performance of the KDE model tends to be better than KCDE and SARIMA during the first few weeks of the season.  Near the beginning of some seasons, predictions from the SARIMA model are quite a bit worse than predictions from the other two component models.  Supplemental Figure [[TODO: Insert figure number]] shows that performance of the component models also varies with the model's uncertainty, as measured by the number of bins required to cover 90\% in the predictive distribution.

Figure \ref{fig:example-weights} displays the estimated component model weights from the \textbf{CW} and \textbf{FW-reg-w} methods for predictions of season onset for each week in the season.  As expected, near the beginning of the season the \textbf{FW-reg-w} ensemble assigns higher weight to the \textbf{KDE} model than the \textbf{CW} ensemble.  The weights assigned to the component model track the relative performance of those models in the training phase fairly well.

## The Best 7 of 9 Models Have Similar Performance in Aggregate

Box and Whiskers plot

We estimated mean performance of each component model for predictions made before the season peak (for predictions of peak incidence or peak timing) or before the season onset (for predictions of season onset) (Figure \ref{fig:}).  

## Ensemble Methods Have More Stable Performance Across Seasons
...
```{r test_phase_log_scores_summary_v1, echo=FALSE, cache=TRUE, fig.height=8, fig.cap="Test phase log scores summary v1.  Log scores across all seasons/season weeks represented in box plots.  Log scores of -Infinity are represented with a cross at -15."}
awes_path <- find.package("awes")
...
```{r test_phase_log_scores_by_season_heatmap_rank_rounded, echo=FALSE, cache=TRUE, fig.height=8, fig.cap="Model performance ranked by mean log score within each of the five test seasons for predictions made before the target (season onset or peak) occurred."}
awes_path <- find.package("awes")
